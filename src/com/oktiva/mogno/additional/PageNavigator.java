...
public class PageNavigator extends com.oktiva.mogno.html.Span {
	...
	throws Exception {
		...
		/* If there is a reminder, we have one page half-full */
		...
		long i=1;
		long end=numPages;
		/* If this is compact, try to place the link in the middle of
		 * all displayed links
		 * else just start @ 0 and go all the way
		 */
		if(isCompact()) {
			long pagesBefore = getMaxDisplayedPages()/2;
			/* This less than, to avoid showing ...
			 * in the first page
			 */
		...
		/* Script and hidden input to submit the form indicated by formSubmit */
		...
		/* Build all links */
		for(;i<end;i++) {
			/* Do not display a link on current page */
			if(i==getCurrentPage()) {
				links.append("<b>");
	...
	private String buildPageLink(long pageNum, String label)
	throws Exception {
		String ret = "";
		A alink = new A();
		alink.setName(getName()+"_link_"+pageNum+"_"+label);
		/* If formSubmit is set, the link calls a JS function */
		if (formSubmit != null && !"".equals(formSubmit)) {
