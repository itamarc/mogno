 ...
 */
public class SuperTable extends com.oktiva.mogno.html.Table {
	...
	public String startColumn(int top, int left, Visual comp) {
		...
		isHeaderCell.contains(""+top+","+left)) {
			s.append("<TH").append(headerPropsHtml).append(getColProperties(left)).append(getCellProperties(top,left));
		} else {
			s.append("<TD").append(getColProperties(left)).append(getCellProperties(top,left));
		}
		s.append(">");
		return s.toString();
	}
