...
public class SimpleTable extends Table {
	...
	throws Exception {
		...
		for (int i = 0; i < columns.length; i++) {
			...
			// If this column is on the hidden columns list
			if ((isHidden & (1<<i))!=0) {
				continue;
			}
		...
		if (dataVector != null) {
			...
			for (int row = 0; row < rows; row++) {
				...
				for (int col = 0; col < cols; col++) {
					...
					// If this column is **not in** the hidden cols list, shows the TD
					if ((isHidden & (1 << col))==0) {
						ret.append("<TD");
						ret.append((String)colProperties.get(col));
						ret.append(">");
						// If this column is **in** the editable cols list, the input is text, not hidden
